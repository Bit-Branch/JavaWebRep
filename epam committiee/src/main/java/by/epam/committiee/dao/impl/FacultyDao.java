...
public class FacultyDao implements Dao<Faculty> {
    ...
    private static final String SELECT_ALL_FACULTIES = "select id,exam_one,exam_two,exam_three from faculty";
    private static final String INSERT_FACULTY = "insert into faculty(id,exam_one,exam_two,exam_three) values (?,?,?,?)";
    private static final String DELETE_FACULTY = "delete from faculty where id = ? ";
    private static final String UPDATE_FACULTY = "UPDATE faculty SET locality = ?,street = ?,building = ?,  flat = ?, zip_code = ?, enrollee_id = ? WHERE id = ?";
    private static final String GET_FACULTY = "select id,exam_one,exam_two,exam_three from faculty WHERE id = ?";
    ...
    public List<Faculty> findAll() throws DaoException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection();
            ...
            while (resultSet.next()) {
                faculties.add(new Faculty(resultSet.getInt(ID_COLUMN),resultSet.getInt(EXAM_ONE_COLUMN),
                        resultSet.getInt(EXAM_TWO_COLUMN), resultSet.getInt(EXAM_THREE_COLUMN)));
            }
    ...
    public void save(Faculty faculty) throws DaoException {
        try (Connection conn = ConnectionPool.getInstance().getConnection();
            ...
            preparedStatement.setInt(2, faculty.getExamOne());
            preparedStatement.setInt(3, faculty.getExamTwo());
            preparedStatement.setInt(4,faculty.getExamThree());
    ...
    public void update(Faculty faculty) throws DaoException {
        try (Connection conn = ConnectionPool.getInstance().getConnection();
            ...
            preparedStatement.setInt(1, faculty.getExamOne());
            preparedStatement.setInt(2, faculty.getExamTwo());
            preparedStatement.setInt(3,faculty.getExamThree());
            preparedStatement.setLong(4, faculty.getId());
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            throw new DaoException(e);
        }
    ...
    public Faculty find(long id) throws DaoException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection();
            ...
            while (resultSet.next()) {
                faculty = new Faculty(resultSet.getInt(ID_COLUMN),resultSet.getInt(EXAM_ONE_COLUMN),
                        resultSet.getInt(EXAM_TWO_COLUMN), resultSet.getInt(EXAM_THREE_COLUMN));
            }
        }catch (SQLException e) {
            throw new DaoException(e);
